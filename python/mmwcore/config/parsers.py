...
_IWR6843_MINIMUM_START_FREQUENCY_HZ = 57e9
_IWR6843_MAXIMUM_START_FREQUENCY_HZ = 64e9
_IWR6843_TRANSMITTER_MASK = 0x07
...
def parse_ti_cli_capture_spec(
    ...
    *,
    ...
    """Parse one IWR6843 TI legacy frameCfg raw capture into a contract."""
...
def _capture_spec(
    ...
    *,
    ...
    _validate_iwr6843_capture_state(state)
...
def _validate_iwr6843_capture_state(state: _TiCliCaptureState) -> None:
    for profile in state.profiles.values():
        if not (
            _IWR6843_MINIMUM_START_FREQUENCY_HZ
            <= profile.start_frequency_hz
            <= _IWR6843_MAXIMUM_START_FREQUENCY_HZ
        ):
            minimum_ghz = _IWR6843_MINIMUM_START_FREQUENCY_HZ / 1e9
            maximum_ghz = _IWR6843_MAXIMUM_START_FREQUENCY_HZ / 1e9
            raise ValueError(
                "IWR6843 profileCfg start frequency must be in "
                f"{minimum_ghz:g}..{maximum_ghz:g} GHz."
            )
    for tx_mask in state.chirp_tx_masks.values():
        if tx_mask < 0 or tx_mask & ~_IWR6843_TRANSMITTER_MASK:
            raise ValueError(
                f"IWR6843 chirpCfg TX mask must use only 0..{_IWR6843_TRANSMITTER_MASK}."
            )
...
def _validated_capture_frame_fields(
    ...
    if not 1 <= rx_mask <= 0x0F:
        raise ValueError("IWR6843 channelCfg RX mask must be in range 1..15.")
    if rx_mask & (rx_mask + 1):
        raise ValueError("Sparse IWR6843 RX masks are not representable by RadarCaptureSpec.")
    if not 1 <= tx_mask <= _IWR6843_TRANSMITTER_MASK:
        raise ValueError(
            f"IWR6843 channelCfg TX mask must be in range 1..{_IWR6843_TRANSMITTER_MASK}."
        )
    if frame_chirp_start < 0 or frame_chirp_end < frame_chirp_start or frame_chirp_end > 511:
        raise ValueError("IWR6843 frameCfg chirp indices must be in range 0..511.")
    if not 1 <= num_loops <= 255:
        raise ValueError("IWR6843 frameCfg num loops must be in range 1..255.")
    if not 0 <= num_frames_raw <= 65535:
        raise ValueError("IWR6843 frameCfg num frames must be in range 0..65535.")
