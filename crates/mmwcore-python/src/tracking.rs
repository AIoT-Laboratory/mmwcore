...
impl NativeClusterTracker2D {
    ...
    fn step<'py>(
        ...
        tracker_step_result_array(py, result)
    }
}
...
impl NativeMeasurementTracker2D {
    ...
    fn step<'py>(
        ...
        let (coordinates, coordinate_shape) =
            tracking_matrix_input(coordinates, "Point coordinates")?;
...
fn native_tracker_config(config: NativeClusterTrackerConfig) -> PyResult<Tracker2DConfig> {
    ...
    .map_err(tracking_error)?;
    let allocation = TrackAllocationConfig::new(
        min_points,
...
fn tracker_step_result_array<'py>(
    ...
    let velocities = Array2::from_shape_vec((track_count, 3), result.velocities)
        ...
        .into_pyarray(py);
    let position_covariances =
        Array3::from_shape_vec((track_count, 2, 2), result.position_covariances)
            .map_err(|_| {
                PyValueError::new_err("Native tracker position covariance shape is invalid.")
            })?
            .into_pyarray(py);
    let extent_covariances = Array3::from_shape_vec((track_count, 2, 2), result.extent_covariances)
